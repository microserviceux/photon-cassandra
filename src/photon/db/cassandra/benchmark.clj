...
(defn test-writes! [db type enc num]
  ...
  (let [start (System/currentTimeMillis)]
    ...
    (let [end (System/currentTimeMillis)
          t-writes (double (/ num (/ (- end start) 1000)))]
      (println "Test for writes" num "|" type "|" enc "| events/s:" t-writes)
      (Thread/sleep 10000)
      (let [rstart (System/currentTimeMillis)]
        (read-all! db enc)
        (let [rend (System/currentTimeMillis)
              t-reads (double (/ num (/ (- rend rstart) 1000)))]
